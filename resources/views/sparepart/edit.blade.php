...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.sparepart.update', $sparepart) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="kategori_id" class="form-select @error('kategori_id') is-invalid @enderror" required>
                        ...
                        @foreach ($kategoris as $k)
                            <option value="{{ $k->id }}" {{ old('kategori_id', $sparepart->kategori_id) == $k->id ? 'selected' : '' }}>{{ $k->nama_kategori }}</option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <select name="satuan" class="form-select @error('satuan') is-invalid @enderror" required>
                        ...
                        <option value="kg" {{ old('satuan', $sparepart->satuan) === 'kg' ? 'selected' : '' }}>Kg</option>
                        <option value="meter" {{ old('satuan', $sparepart->satuan) === 'meter' ? 'selected' : '' }}>Meter</option>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="harga_beli" class="form-control @error('harga_beli') is-invalid @enderror"
                        value="{{ old('harga_beli', $sparepart->harga_beli) ? number_format(old('harga_beli', $sparepart->harga_beli), 0, ',', '.') : '' }}"
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="harga_jual" class="form-control @error('harga_jual') is-invalid @enderror"
                        value="{{ old('harga_jual', $sparepart->harga_jual) ? number_format(old('harga_jual', $sparepart->harga_jual), 0, ',', '.') : '' }}"
                        inputmode="numeric" onkeyup="this.value=formatRupiah(this.value)">
            ...
            <div class="mb-3">
                ...
                <textarea name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" rows="3">{{ old('deskripsi', $sparepart->deskripsi) }}</textarea>
